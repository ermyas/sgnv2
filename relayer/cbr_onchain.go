...
func (c *CbrOneChain) monLiqAdd(blk *big.Int) {
	...
	c.mon.Monitor(cfg, func(id monitor.CallbackID, eLog ethtypes.Log) (recreate bool) {
		...
		if err != nil {
			log.Warnf("UpsertLP err: %s, nonce %d, amt %s, txHash %x, chainId %d", err, nonce, ev.Amount.String(), eLog.TxHash, c.chainid)
			return false
		}
		return false
	})
...
func (c *CbrOneChain) monWithdraw(blk *big.Int) {
	...
	c.mon.Monitor(cfg, func(id monitor.CallbackID, eLog ethtypes.Log) (recreate bool) {
		...
		GatewayOnLiqWithdraw(c.chainid, ev.Seqnum, ev.Receiver.String(), eLog.TxHash.String())
		return false
	})
}

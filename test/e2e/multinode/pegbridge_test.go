...
func pegbridgeTest(t *testing.T) {
	...
	log.Infoln("======================== Deposit ===========================")
	supplyCap := tc.GetSupplyCap(transactor, tc.CbrChain1.ChainId, tc.CbrChain2.ChainId, tc.CbrChain2.PeggedUNIAddr)
	log.Infoln("Supply cap: ", supplyCap)
	tc.CheckTotalSupply(transactor, tc.CbrChain1.ChainId, tc.CbrChain2.ChainId, tc.CbrChain2.PeggedUNIAddr, "0")
	...
	tc.CheckTotalSupply(transactor, tc.CbrChain1.ChainId, tc.CbrChain2.ChainId, tc.CbrChain2.PeggedUNIAddr, mintAmt.String())
	...
	err = tc.FakeStartClaimPegbridgeRefund(transactor, depositId, tc.CbrChain1.Users[1])
	tc.ChkErr(err, "user1 tried to claim pegbridge refund")
	log.Infoln("user1 failed to claim refund of user0's invalid deposit")
	err = tc.StartClaimPegbridgeRefund(transactor, depositId, tc.CbrChain1.Users[0])
	...
	err = tc.FakeStartClaimPegbridgeRefund(transactor, depositId, tc.CbrChain1.Users[0])
	...
	expectedTotalSupply := new(big.Int).Add(mintAmt, new(big.Int).Neg(burnAmt))
	tc.CheckTotalSupply(transactor, tc.CbrChain1.ChainId, tc.CbrChain2.ChainId, tc.CbrChain2.PeggedUNIAddr, expectedTotalSupply.String())

...
func CanWithdrawInvariant(k Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		valDelegationAddrs := make(map[string][]eth.Addr)
		...
		k.stakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
			_, _ = k.WithdrawValidatorCommission(ctx, val.GetEthAddr())

			delegationAddrs, ok := valDelegationAddrs[val.GetOperator().String()]
			if ok {
				for _, delAddr := range delegationAddrs {
					if _, err := k.WithdrawDelegationRewards(ctx, delAddr, val.GetEthAddr()); err != nil {
						panic(err)
					}
				}
			}

			remaining = k.GetValidatorOutstandingRewardsCoins(ctx, val.GetEthAddr())

...
func (k Keeper) GetWithdrawableBalance(ctx sdk.Context, delAddr eth.Addr, denom string) sdk.Coin {
	derivedAccAddress := common.DeriveSdkAccAddressFromEthAddress(types.ModuleName, delAddr)
	return k.bankKeeper.GetBalance(ctx, derivedAccAddress, denom)
}
...
func (k Keeper) ClaimMessageFees(ctx sdk.Context, delAddr eth.Addr) error {
	// 1. Withdraw reward for all validators. If delAddr is a validator address, withdraw its commission.
	err := k.withdrawRewardsAndCommission(ctx, delAddr)
	if err != nil {
		return err
	...
	// 2. Emit claim_message_fees event
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		types.EventTypeClaimMessageFees,
		sdk.NewAttribute(types.AttributeKeyDelegatorAddress, delAddr.String()),
	))

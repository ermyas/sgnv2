...
func (keeper Keeper) AddDeposit(ctx sdk.Context, proposalID uint64, depositorAddr sdk.AccAddress, depositAmount sdk.Int) (bool, error) {
	...
	if found {
		...
		deposit = types.NewDeposit(proposalID, depositorAddr.String(), depositAmount)
	}
...
func (keeper Keeper) RefundDeposits(ctx sdk.Context, proposalID uint64) {
	...
	keeper.IterateDeposits(ctx, proposalID, func(deposit types.Deposit) bool {
		acc, _ := sdk.AccAddressFromBech32(deposit.Depositor)
		accTotalDeposit, found := keeper.GetAccTotalDeposit(ctx, acc)
		if !found {
			log.Errorf("deposit not found. %s", deposit)
			return false
		}
		accTotalDeposit.Amount = accTotalDeposit.Amount.Sub(deposit.Amount)
		keeper.SetAccTotalDeposit(ctx, acc, accTotalDeposit)

		store.Delete(types.DepositKey(proposalID, acc))
		return false
	})
}

	...
	if csType == webapi.CSType_CT_TX {
		...
		if transfer != nil {
			if transfer.Status == types.TransferHistoryStatus_TRANSFER_COMPLETED || transfer.Status == types.TransferHistoryStatus_TRANSFER_DELAYED {
				...
				err = onchain.GatewayOnRelay(gs.Chains.GetEthClient(uint64(chainId)), transfer.TransferId, "",
					relayOnChain.GetRelayOnChainTransferId().String(), new(big.Int).SetBytes(relayOnChain.GetAmount()).String(),
					eth.Bytes2Addr(relayOnChain.GetReceiver()).String(), eth.Bytes2Addr(relayOnChain.GetToken()).String(),
					uint64(chainId), ev.DstChainId, dal.BridgeTypeSendRelay)
				if err != nil {
					return err
				}
			}
